...
image:
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=adoptopenjdk/openjdk11:jdk-11.0.26_4-centos --build-arg DETEMPLATIZE_IMAGE_VERSION=$(DETEMPLATIZE_IMAGE_VERSION) -t $(IMAGE_NAME) . # Build image and automatically tag it as latest on jdk17
