...
image:
	...
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=${JDK17_BASE_IMG} --build-arg DETEMPLATIZE_IMAGE_VERSION=$(DETEMPLATIZE_IMAGE_VERSION) --build-arg TOMCAT_MAJOR_VERSION=9 -t $(IMAGE_NAME) . # Build image and automatically tag it as latest on jdk17
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=${JDK11_BASE_IMG} --build-arg DETEMPLATIZE_IMAGE_VERSION=$(DETEMPLATIZE_IMAGE_VERSION) --build-arg TOMCAT_MAJOR_VERSION=9 -t $(IMAGE_NAME):4-jdk11 . # Build image using tomcat 9 , jdk 11
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=${JDK17_BASE_IMG} --build-arg DETEMPLATIZE_IMAGE_VERSION=$(DETEMPLATIZE_IMAGE_VERSION) --build-arg TOMCAT_MAJOR_VERSION=9 -t $(IMAGE_NAME):4-jdk17 . # Build image using tomcat 9 , jdk 17
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=${JDK21_BASE_IMG} --build-arg DETEMPLATIZE_IMAGE_VERSION=$(DETEMPLATIZE_IMAGE_VERSION) --build-arg TOMCAT_MAJOR_VERSION=10 -t $(IMAGE_NAME):4-jdk21 . # Build image using tomcat 10 , jdk 21

test: image
	# Build image for executing test cases against it
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=${JDK11_BASE_IMG} -t qualitytest . --target qualitytest
	# Execute test cases
