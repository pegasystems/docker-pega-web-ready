...
image:
	...
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=docker-dev.bin.pega.io/pega.com/tomcat-ready:v9-jdk17 --build-arg DETEMPLATIZE_IMAGE_VERSION=$(DETEMPLATIZE_IMAGE_VERSION) --build-arg TOMCAT_MAJOR_VERSION=9 -t $(IMAGE_NAME) . # Build image and automatically tag it as latest on jdk17
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=docker-dev.bin.pega.io/pega.com/tomcat-ready:v9-jdk11 --build-arg DETEMPLATIZE_IMAGE_VERSION=$(DETEMPLATIZE_IMAGE_VERSION) --build-arg TOMCAT_MAJOR_VERSION=9 -t $(IMAGE_NAME):3-jdk11 . # Build image using tomcat 9 , jdk 11
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=docker-dev.bin.pega.io/pega.com/tomcat-ready:v9-jdk17 --build-arg DETEMPLATIZE_IMAGE_VERSION=$(DETEMPLATIZE_IMAGE_VERSION) --build-arg TOMCAT_MAJOR_VERSION=9 -t $(IMAGE_NAME):3-jdk17 . # Build image using tomcat 9 , jdk 17
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=docker-dev.bin.pega.io/pega.com/tomcat-ready:v10-jdk21 --build-arg DETEMPLATIZE_IMAGE_VERSION=$(DETEMPLATIZE_IMAGE_VERSION) --build-arg TOMCAT_MAJOR_VERSION=10 -t $(IMAGE_NAME):3-jdk21 . # Build image using tomcat 10 , jdk 21

test: image
	# Build image for executing test cases against it
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=docker-dev.bin.pega.io/pega.com/tomcat-ready:v9-jdk11 -t qualitytest . --target qualitytest
	# Execute test cases
