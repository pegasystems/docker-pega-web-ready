...
if [ "$JDBC_DRIVER_URI" != "" ]; then
  if ! ls "${lib_root}"/*.jar 1>/dev/null 2>&1; then
    echo "WARNING: JDBC_DRIVER_URI is set but no JAR files found in ${lib_root}. Mount the JDBC driver JAR into ${lib_root} before starting the container."
  fi
fi
